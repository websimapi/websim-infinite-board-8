...
import { BoardGenerator, CHUNK_DEPTH } from './board-gen.js';
...
export class Game {
    constructor() {
        ...
        this.chunkUpdateTimer = 0; // throttle for view-based chunk updates
    ...
    checkChunkLoad() {
        ...
        // Player-based chunk index
        const playerChunkIdx = this.getChunkIndexFromNodeId(this.currentNode.id);

        // Camera/pan-based chunk index (camera looks toward negative Z)
        const focusZ = this.graphics.panFocus.z;
        const focusChunkIdx = Math.round(-focusZ / CHUNK_DEPTH);

        const minIdx = Math.min(playerChunkIdx, focusChunkIdx);
        const maxIdx = Math.max(playerChunkIdx, focusChunkIdx);

        // Load a band of chunks that covers both player and camera view,
        // with a little padding behind and further padding ahead.
        const loadMin = minIdx - 2;
        const loadMax = maxIdx + 4;

        for (let i = loadMin; i <= loadMax; i++) {
            this.ensureChunk(i);
        }

        // Prune anything far outside both player and camera interest
        this.graphics.pruneChunks(loadMin, loadMax);
    }
    ...
    animate() {
        ...
        // Periodically update chunk loading based on both player and camera position,
        // so panning the view will stream in nearby chunks.
        if (this.chunkUpdateTimer > 0.3) { // every ~0.3s
            this.checkChunkLoad();
